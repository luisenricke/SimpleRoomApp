...
public class DateOperation {
    ...
    public static Date[] getNowAndPast(byte optionTime, int backwardTimeCount) throws IllegalArgumentException {
        ...
        if (backwardTimeCount > 0) {
            if (optionTime == 0)
                dates[1] = DateOperation.subMinute(dates[0], backwardTimeCount);
            else if (optionTime == 1)
                dates[1] = DateOperation.subHour(dates[0], backwardTimeCount);
            else if (optionTime == 2)
                dates[1] = DateOperation.subDay(dates[0], backwardTimeCount);
            else if (optionTime == 3)
                dates[1] = DateOperation.subMonth(dates[0], backwardTimeCount);
            else if (optionTime == 4)
                dates[1] = DateOperation.subYear(dates[0], backwardTimeCount);
            else
